...
export default function CurrentUserTimeActual() {
    ...
    const groupedActuals = actuals.reduce((acc: Record<string, { actualName: string; project: string; projectId: string; actuals: Actual[] }>, actual) => {
    ...
    const getMonthMetrics = (month: string) => {
        ...
        Object.entries(groupedActuals).forEach(([key, group]) => {
            group.actuals.forEach((actual: Actual) => {
                if (actual.dateEx) {
                    const actualYm = getYearMonthFromDate(actual.dateEx);
                    if (actualYm === month) {
        ...
        actualList.forEach((actual: Actual) => {
            if (actual.dateEx) {
                ...
                if (monthRange.includes(actualYm)) {
                    ...
                    // Sum amounts
                    result[actualYm].amount += actual.amount || 0;
                    // Mark as done if ANY actual in this month is done
                    if (actual.done) {
                        result[actualYm].done = true;
                    }
                }
            }
        });
    ...
    return (
        <div className="flex flex-col gap-2 p-6 h-screen">
            <TimeMapHeader title="My Assigned Actuals" nRangeMonth={nRangeMonth} onRangeMonthChange={handleRangeMonthChange} />
                                                ...
                                                <td className="py-2 px-3 font-medium text-gray-900 bg-white sticky left-0 z-10">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div className="flex flex-col gap-1">
                                                            <div className="text-blue-600 hover:text-blue-800 hover:underline text-left font-medium">{group.actualName}</div>
